#script that tests client/views.py methods
#it assumes that there are two servers runing
#one in port 8000 and another in port 8001
...
#    command = "curl -v -c %s -b %s -d \"username=a&password=a&amicat=True&csrfmiddlewaretoken=%s\" %s"%(COOKIEFILE,COOKIEFILE,cookie, base_url + function)
    command = "%s -v -c %s -b %s -d %s %s"% (CURL, COOKIEFILE, COOKIEFILE, s, base_url + function)
    print command
    os.system(command)
